#property copyright "René Balke 2021"
#property link      "www.mqltrading.de"
...
void OnTick()
{
   ...
   if(totalBars != bars)
   {
      ...
      if(stoch[1] > signal[1] && stoch[0] < signal[0])
      {
         ...
         if(stoch[1] <= StochLowerBound ||
            signal[1] <= StochLowerBound)
         {
            Print("Buy Signal");
      ...
      else if(stoch[1] < signal[1] && stoch[0] > signal[0])
      {
         ...
         if(stoch[1] >= StochUpperBound ||
            signal[1] >= StochUpperBound)
         {
            Print("Sell Signal");
